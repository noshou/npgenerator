...
public class OctagonalPrism extends Shape {
    ...
    private final Apfloat EIGHT = new Apfloat("8", super.precision);


    /**
     * Constructs a new {@code Shapes.OctagonalPrism} instance with the given parameters.
     */
    public OctagonalPrism (
        ...
        // Constants
        Apfloat SQRT2 = ApfloatMath.sqrt(TWO);
        ...
        Apfloat sqrt2_minus_sqrt2 = ApfloatMath.sqrt(TWO.subtract(SQRT2));
        Apfloat factor = ONE.add(TWO.subtract(SQRT2).divide(FOUR));
        Apfloat r_oct = r.divide(ApfloatMath.sqrt(factor));
        Apfloat h = r_oct.multiply(sqrt2_minus_sqrt2).divide(TWO);
        ...
        Triad<Apfloat> vBot0 = new Triad<>(r_oct.multiply(cos_vals[0]), r_oct.multiply(sin_vals[0]), h.multiply(NEG));
        Triad<Apfloat> vBot1 = new Triad<>(r_oct.multiply(cos_vals[1]), r_oct.multiply(sin_vals[1]), h.multiply(NEG));
        Triad<Apfloat> vBot2 = new Triad<>(r_oct.multiply(cos_vals[2]), r_oct.multiply(sin_vals[2]), h.multiply(NEG));
        Triad<Apfloat> vBot3 = new Triad<>(r_oct.multiply(cos_vals[3]), r_oct.multiply(sin_vals[3]), h.multiply(NEG));
        Triad<Apfloat> vBot4 = new Triad<>(r_oct.multiply(cos_vals[4]), r_oct.multiply(sin_vals[4]), h.multiply(NEG));
        Triad<Apfloat> vBot5 = new Triad<>(r_oct.multiply(cos_vals[5]), r_oct.multiply(sin_vals[5]), h.multiply(NEG));
        Triad<Apfloat> vBot6 = new Triad<>(r_oct.multiply(cos_vals[6]), r_oct.multiply(sin_vals[6]), h.multiply(NEG));
        Triad<Apfloat> vBot7 = new Triad<>(r_oct.multiply(cos_vals[7]), r_oct.multiply(sin_vals[7]), h.multiply(NEG));
        ...
        Triad<Apfloat> top_norm = new Triad<>(new Apfloat("0", precision), new Apfloat("0", precision), ONE);
        ...
        Triad<Apfloat> bottom_norm = new Triad<>(new Apfloat("0", precision), new Apfloat("0", precision), NEG);
        ...
        Triad<Apfloat> side0_norm = VectorMath.normalQuad(vTop0, vBot0, vBot1, vTop1, true);

        Tetrad<Tuple<Apfloat>> side1 = new Tetrad<>(vTop1, vBot1, vBot2, vTop2);
        Triad<Apfloat> side1_norm = VectorMath.normalQuad(vTop1, vBot1, vBot2, vTop2, true);

        Tetrad<Tuple<Apfloat>> side2 = new Tetrad<>(vTop2, vBot2, vBot3, vTop3);
        Triad<Apfloat> side2_norm = VectorMath.normalQuad(vTop2, vBot2, vBot3, vTop3, true);

        Tetrad<Tuple<Apfloat>> side3 = new Tetrad<>(vTop3, vBot3, vBot4, vTop4);
        Triad<Apfloat> side3_norm = VectorMath.normalQuad(vTop3, vBot3, vBot4, vTop4, true);

        Tetrad<Tuple<Apfloat>> side4 = new Tetrad<>(vTop4, vBot4, vBot5, vTop5);
        Triad<Apfloat> side4_norm = VectorMath.normalQuad(vTop4, vBot4, vBot5, vTop5, true);

        Tetrad<Tuple<Apfloat>> side5 = new Tetrad<>(vTop5, vBot5, vBot6, vTop6);
        Triad<Apfloat> side5_norm = VectorMath.normalQuad(vTop5, vBot5, vBot6, vTop6, true);

        Tetrad<Tuple<Apfloat>> side6 = new Tetrad<>(vTop6, vBot6, vBot7, vTop7);
        Triad<Apfloat> side6_norm = VectorMath.normalQuad(vTop6, vBot6, vBot7, vTop7, true);

        Tetrad<Tuple<Apfloat>> side7 = new Tetrad<>(vTop7, vBot7, vBot0, vTop0);
        Triad<Apfloat> side7_norm = VectorMath.normalQuad(vTop7, vBot7, vBot0, vTop0, true);
    ...
    protected boolean inBounds(@NotNull Triad<Apfloat> point_cart) {
        ...
        for (int i = 0; i < faces.fetchSize(); i++) {
            ...
            if (i < 2) {  // Octagonal faces (top and bottom)
                ...
                centroid = new Triad<>(sum_x.divide(EIGHT), sum_y.divide(EIGHT), sum_z.divide(EIGHT));
                ...
                centroid = new Triad<>(
                        v0.fetch(0).add(v1.fetch(0)).add(v2.fetch(0)).add(v3.fetch(0)).divide(FOUR),
                        v0.fetch(1).add(v1.fetch(1)).add(v2.fetch(1)).add(v3.fetch(1)).divide(FOUR),
                        v0.fetch(2).add(v1.fetch(2)).add(v2.fetch(2)).add(v3.fetch(2)).divide(FOUR)
                );
            }
